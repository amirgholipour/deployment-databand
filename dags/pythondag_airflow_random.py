...
def select_one_year(alldata):
    random_year = randrange(1965,2025)
    with dataset_op_logger(motogp_file,
                            "read",
                            with_schema=True,
                            with_preview=True,
        ...
        oneyear = alldata[alldata.Season.eq(random_year)]
...
with DAG(
    dag_id="Python_Airflow_DAG_random",
    ...
    motogp_dag_python_random_airflow = PythonOperator (
        task_id="Task_Group",
        python_callable=motogp_pipeline
    )

motogp_dag_python_random_airflow